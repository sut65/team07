...
func GetVehicleInspection(c *gin.Context) {
	var vehicleinspection entity.VehicleInspection
	id := c.Param("id")
	if err := entity.DB().Preload("Ambulance").Preload("StatusCheck").Preload("Employee").Preload("Employee.User").Preload("Employee.User.Role").Preload("AmbulancePart").Preload("Ambulance.TypeAbl").Raw("SELECT * FROM vehicle_inspections WHERE id = ?", id).Find(&vehicleinspection).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListVehicleInspections(c *gin.Context) {
	...
	if err := entity.DB().Preload("Ambulance").Preload("StatusCheck").Preload("Employee").Preload("Employee.User").Preload("Employee.User.Role").Preload("AmbulancePart").Preload("Ambulance.TypeAbl").Raw("SELECT * FROM vehicle_inspections").Find(&vehicleinspection).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": vehicleinspection})
